...
import { getExpenses, createExpense, getFinancialSummary, type ExpenseRecord, type ExpenseInput, type ExpenseCategory } from '../../lib/financeService';
...
const CATEGORIES: { value: ExpenseCategory; label: string }[] = [
    { value: 'BENEFICIARY_AID', label: 'Beneficiary Aid' },
...
const FinanceTab: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const [exp, sum] = await Promise.all([getExpenses(), getFinancialSummary()]);
            setExpenses(exp);
            setSummary(sum);
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        setShowForm(false);
        setForm({ description: '', amount: 0, category: 'OPERATIONS' });
        await load();
    };
            ...
            <div className="grid sm:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-[#111827] p-5 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Inflow</p>
                    </div>
                ...
                <div className="bg-white dark:bg-[#111827] p-5 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Outflow</p>
                    </div>
                ...
                <div className="bg-white dark:bg-[#111827] p-5 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Net Balance</p>
                    </div>
                            ...
                            return (
                                <div key={cat}>
                                    <div className="flex justify-between text-sm mb-1">
                                        <span className="text-slate-700 dark:text-slate-300 font-medium">{cat.replace(/_/g, ' ')}</span>
                                        <span className="text-slate-500 dark:text-slate-400">{formatNaira(amount)} ({pct}%)</span>
                                    </div>
                ...
                <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-white/10 shadow-sm p-6">
                    ...
                    <form onSubmit={handleCreate} className="grid sm:grid-cols-2 gap-4">
                        ...
                        <div className="sm:col-span-2 flex justify-end">
                            <button type="submit" className="px-6 py-2.5 bg-primary-600 dark:bg-primary-500 text-white rounded-lg font-medium text-sm hover:bg-primary-700 transition shadow-sm">Save Expense</button>
                        </div>
                    </form>
                </div>
            )}
                ...
                <div className="divide-y divide-slate-100 dark:divide-white/5">
                    {expenses.length === 0 ? (
                        ...
                        <div key={e.id} className="p-5 flex items-center justify-between gap-4">
                            <div>
                                <h3 className="font-bold text-slate-900 dark:text-white">{e.description}</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{formatDate(e.createdAt)}</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <StatusBadge status={e.category} />
                                <p className="text-lg font-bold text-red-600 dark:text-red-400 tabular-nums">-{formatNaira(e.amount)}</p>
                            </div>
                        </div>
                    ))}
                </div>
